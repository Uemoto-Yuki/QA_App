...
class QuestionDetailListAdapter(context: Context, private val mQustion: Question) : BaseAdapter() {
    companion object {
        //定数を2つ用意。どのレイアウトを使って表示させるかを判断するためのタイプを表す定数。
        //今回は質問と回答の2つのレイアウトを使うので2つの定数を用意。もしこれが3つであればもう1つ定数を用意する。
    ...
    //ソッドは引数で渡ってきたポジションがどのタイプかを返す
    override fun getItemViewType(position: Int): Int {
        return if (position == 0) { //1行目、つまりポジションが0の時に質問であるTYPE_QUESTIONを返し
            TYPE_QUESTION
        } else { // それ以外は回答なのでTYPE_ANSWERを返す
            TYPE_ANSWER
        }
    }
